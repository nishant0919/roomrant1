...
import { AiOutlineExperiment, AiOutlineFileAdd } from "react-icons/ai";
...
function AddMain() {
  ...
  const [imageOption, setImageOption] = useState("cover");
  ...
  async function validData() {
    ...
    if (errorMsg) {
      ...
      const res = await uploadtoIbb(image);
      setUploadingStatus("Image Uploaded");
      setUserData({ ...userData, image: res });
      const sendData = { ...userData, image: res };
      uploadData(sendData);
    }
  }

  function uploadData(data) {
    setUploadingStatus("Uploading Data to server");
    fetch("/api/add", {
      method: "POST",
      ...
      .then((data) => {
        if (data.message === "Room added successfully") {
          setUploadingStatus("Room Added Successfully");
          setTimeout(() => {
            setUploading(false);
  ...
  return (
    <div className="w-full relative flex justify-center p-5 bg-[#fafafa] min-h-screen py-10 overflow-scroll">
      {uploading && (
        <div className="fixed flex justify-center items-center z-50 top-0 left-0 bg-black bg-opacity-80 backdrop-blur-sm h-full w-full">
          <div className="flex">
      ...
      <div className="flex flex-col gap-3 md:w-2/3 w-full bg-[#ffffff] rounded-md shadow-md p-3 h-full">
        <h2 className="text-2xl font-bold">Add a Room.</h2>
        <p className="opacity-60">
          Please fill the form below with genuine information. Do not mislead
          others.
        </p>
        <p className="text-xl font-bold mt-5">Room Image</p>
        <div className="flex w-full p-4">
          {(!image && (
            <div
              ...
              className="flex border border-dashed border-black rounded-md h-52 w-full hover:bg-gray-300 cursor-pointer duration-300 items-center justify-center"
            >
              <AiOutlineFileAdd className="text-4xl" />
              <p>Upload an Image</p>
              <input
                onChange={(e) => {
            ...
            <div className="flex border border-dashed border-black rounded-md h-52 w-full duration-300 items-center justify-center relative">
              <div
                onClick={() => {
                  setImage(null);
                }}
                className="flex absolute right-2 top-2 h-8 w-8 rounded-full bg-red-800 cursor-pointer hover:bg-red-400 duration-300 text-white justify-center items-center"
              >
                x
              </div>
              <div
                onClick={() => {
                  setImageOption(imageOption === "cover" ? "contain" : "cover");
                }}
                className="flex absolute right-12 top-2 h-8 w-8 rounded-full bg-blue-800 cursor-pointer hover:bg-blue-400 duration-300 text-white justify-center items-center"
              >
                {imageOption === "cover" ? "C" : "O"}
              </div>
              <img
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                onChange={(e) => handleChange(e)}
                name="location"
                placeholder="Hattiban, House No. 26, Lalitpur Nepal"
                className="p-2 border border-gray-300 rounded-md"
              />
            </div>
            <div className="flex flex-col gap-1">
            ...
            <div className="flex flex-col gap-1">
              ...
              <Select
                ...
                isMulti
                onChange={(e) => {
                  setUserData({ ...userData, features: e.map((i) => i.value) });
                }}
            ...
            <button
              onClick={validData}
              className=" p-[20px] bg-gray-100 text-black rounded-md"
            >
              Submit
            </button>
          </div>
