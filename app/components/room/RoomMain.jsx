...
function RoomMain({ data, id, booked }) {
  ...
  const isAuth = status === "authenticated" && sessionData?.user?.email === data?.author?.email;
  ...
  return (
    <div className="flex relative flex-col md:flex-row gap-2 min-h-screen">
      ...
      <div className="flex flex-col justify-start md:w-1/3 w-full overflow-hidden">
          ...
          <div className="flex items-center -mx-4 space-x-2 overflow-x-auto overflow-y-hidden sm:justify-center flex-nowrap">
            <a
              ...
              className={`flex items-center flex-shrink-0 duration-300 transition-all cursor-pointer hover:border-gray-400 px-5 py-2 border-b-4 ${
                active === "description" ? "border-purple-600" : "border-gray-300"
              } text-gray-600`}
            >
              Description
            </a>
        ...
        {active === "location" && (
          <RmMap
            address={data?.location || "Kachankawal Digital"}
          />
        )}
        {active === "owner" && (
          <motion.div
          ...
          >
            ...
            />
            <div>
              <h2 className="text-xl font-semibold">{data?.author?.name || "Unknown Owner"}</h2>
            ...
            {booked?.length > 0 ? (
              <div className="flex flex-col gap-2">
                {booked.map((book, index) => (
                  <div key={index} className="container flex flex-col w-full max-w-lg p-2 mx-auto divide-y rounded-md dark:divide-gray-300 bg-purple-200 mt-5">
                      ...
                      <div className="flex space-x-4">
                        <div>
                          <img
                            src={book?.user?.image || ""}
                            referrerPolicy="no-referrer"
                            alt={book?.user?.name || "Booker"}
                            className="object-cover w-12 h-12 rounded-full dark:bg-gray-500"
                          />
                        </div>
                        <div>
                          <h4 className="font-bold">{book?.user?.name || "Unknown"}</h4>
                          <span className="text-xs dark:text-gray-600">
                            {book?.user?.email || "Unknown Email"}
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="flex flex-col p-2 gap-3">
                      <span className="text-sm dark:text-gray-600">
                        Booked On: {moment(book?.createdAt).fromNow() || "Unknown"}
                      </span>
                      <button
                        onClick={() => window.open(`mailto:${book?.user?.email || ""}`)}
